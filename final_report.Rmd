...
```{r comm-data-3}
data_from_to <- comm_data %>%
  group_by(from, to, day) %>%
  count()
ggplot(data_from_to, aes(x = to, y = n, color = day)) +
  geom_point() +
  labs(title = "All Communications by Sender and Receiver Pairs")
```
